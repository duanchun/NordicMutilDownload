...
class MyFrame(wx.Frame):
    ...
    def targetFileSelect(self):
        ...
        # Process Bar
        self.processBarOne = PG.PyGauge(self.panel, size=(350, 20))
        self.processBarOne.SetBackgroundColour(wx.WHITE)
        self.processBarOne.SetBorderColor(wx.BLACK)
        self.processBarOne.SetBarColour(wx.GREEN)
        self.processBarOne.SetValue(0)
        self.processBarOneTips = wx.StaticText(self.processBarOne,label='%0',style=wx.ALIGN_CENTER,pos=(2,2),size=(350,10))
        self.processBarOneTips.SetBackgroundColour(wx.RED_BRUSH)
        ...
        # Process Bar
        self.processBarTwo = wx.Gauge(self.panel, range=TASK_RANGE, size=(350, 20))
        #self.processBarTwo.SetValue(1)
    ...
    def onBtnDownload(self,event):
        ...
        if self.serialSelect.GetValue():
            try:
                ...
                self.GaugeProcessIndexOne += 1
                self.processBarOne.Update(100,10000)
            except:
                print("Error")
    ...
    def downloadProcessTimerHandle(self, event):
        ...
        if -1 != self.GaugeProcessIndexOne:
            self.GaugeProcessIndexOne += 1
            #self.processBarOne.SetValue(self.GaugeProcessIndexOne)
            #self.processBarOne.Update(self.GaugeProcessIndexOne,21)
        if -1 != self.GaugeProcessIndexTwo:
            self.GaugeProcessIndexTwo += 1
            self.processBarTwo.SetValue(self.GaugeProcessIndexTwo)
    ...
    def updateUI(self,msg):
        ...
        if DOWNLOAD_EVT_OVER == param[0]:
            ...
            if self.downloadTaskNumRecodFlag <= 0:
                self.btnDownload.Enable()
                if -1 != self.GaugeProcessIndexOne:
                    reliveIndex = self.processBarOne.GetValue()
                    print("reliveIndex==",reliveIndex)
                    self.processBarOne.SetBarColour(wx.RED)
                    self.processBarOne.Update(TASK_RANGE - reliveIndex,50)
                if -1 != self.GaugeProcessIndexTwo:
                    self.processBarTwo.SetValue(TASK_RANGE)

                #self.timer.Stop()
        elif DOWNLOAD_EVT_SERIAL_ERROR_HAPPEND == param[0]:
            if self.serialSelect.GetValue() == param[1]:
                self.processBarOne.SetValue(TASK_RANGE)
                self.GaugeProcessIndexOne = -1
            ...
            if isinstance(param,int):
                self.processBarTwo.SetValue(param)
